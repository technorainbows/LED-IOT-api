(function() {
    ...
    function startLoadingPage() {
        ...
        $(document).on("click", ".btn-device", function selectDevice() {
            ...
            if (lastDevice != deviceID) {
                console.debug(
                    "lastDevice != deviceID ---> " + lastDevice +
                    " != " + deviceID
                );
                ...
                if (
                    lastDevice &&
                    document.getElementById(lastDevice).classList.contains(
                        "btn-current")
                ) {
    ...
    function updateDeviceList(newDevices) {
        ...
        for (let device of newDevices) {
            if (!deviceSet.has(device)) add.push(device);
    ...
    async function insertDevice(device) {
        ...
        try {
            ...
            deviceName = await $apiUtils.getParam(apiUrl, device, "name");
        } catch (error) {
